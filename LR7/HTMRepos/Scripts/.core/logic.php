...
class Filter
{
    ...
    private function DrawRow($row) : void
    {
        ...
        <form action='UpdateOrderPage.php' method='post'>
            <input type='hidden' name='nameOfSet' value=\"$row[photo]\">
            <input type='hidden' name='fio' value=\"$row[personal_info]\">
            <input type='hidden' name='content' value=\"$row[content]\">
            <input type='hidden' name='cost' value=\"$row[cost]\">
            <input type='hidden' name='tradeOutlet' value=\"$row[address]\">
            <td> <button class='blue-button' type='submit'>Update</button> </td>
        </form>
    
        <form action='DeleteOrderPage.php' method='post'>
            <td> <button class='red-button' name='Delete' value=\"$row[photo]\" type='submit'>Delete</button></td>
        </form>
             ";
    }
...
//                    $name = strtr($name, $converter);
                    ...
                    {
                        $error = 'Недопустимое разрешение файла';
    ...
    {
        if($_POST["nameOfSet"] !== "" && $_POST["content"] !== "" && $_POST["cost"] !== "")
        {
            $query = $GLOBALS["dbMain"]->GetPDO()->prepare(
                "select ID from orders where (photo = :photo and personal_info = :personal_info);"
            );

            $query->bindValue(":photo", $_POST['nameOfSet']);
            $query->bindValue(":personal_info", $_POST['fio']);

            if (!$query->execute()) {
                throw new PDOException('Мы где-то просчитались(');
            }
    ...
    public static function UpdateOrder($ID)
    {
        if(isset($_POST))
        {
            if($_POST["nameOfSet"] !== "" && $_POST["content"] !== "" && $_POST["cost"] !== "")
            {
                $tradeOutlet = $_POST["tradeOutlet"];

                $query = $GLOBALS["dbMain"]->GetPDO()->prepare(
                    "update orders set photo = :photo, personal_info = :personal_info, trade_outlet = :trade_outlet,
                  content = :content, cost = :cost where (ID = $ID);"
                );
                ...
                if(isset($_FILES["photo"]) && $_FILES["photo"]["name"] !== "")
                {
                    ...
                    $photoName = TableModel::GetPhotoByID($ID);
                    $filePath = "../../SushiPictures/".$photoName;
                ...
                $query->bindValue(":personal_info", $_POST['fio']);
                $query->bindValue(":trade_outlet", $tradeOutlet, PDO::PARAM_INT);
                $query->bindValue(":content", $_POST['content']);
                $query->bindValue(":cost", $_POST['cost']);

                if(!$query->execute())
                {
                    throw new PDOException('Мы снова где-то просчитались(');
                }
            }
        }
    ...
    public static function DeleteOrder()
    {
        if(isset($_POST))
        {
            ...
            {
                $query = $GLOBALS["dbMain"]->GetPDO()->prepare(
                    "Delete from orders where photo = :photo;");

                $query->bindValue(":photo", $_POST["Delete"]);


                if(!$query->execute())
                {
                    throw new PDOException('При удалении возникла ошибка');
                }

                $filePath = "../../SushiPictures/".$_POST["Delete"];
    ...
    private static function GetPhotoByID($ID):string
    {
        $query = "select photo from orders where ID = $ID;";
        $result = $GLOBALS["dbMain"]->ExecuteSQL($query);
        $result = $result->fetch();
